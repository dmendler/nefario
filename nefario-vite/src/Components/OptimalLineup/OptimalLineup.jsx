...
const OptimalLineup = () => {
  ...
  const handleOpponentRelayChange = (
    eventId,
    relayIndex,
    swimmerIndex,
    value
    ...
    for (const event of events) {
      if (event.type === "individual") {
        ...
        if (entries.length < 3 || entries.some((time) => !time)) {
          setWarning(
            `Please enter 3 valid opponent swimmer times for ${event.name}.`
          );
        ...
        if (
          entries.length < 3 ||
          entries.some(
            (relay) => relay.length < 4 || relay.some((time) => !time)
          )
        ) {
          setWarning(
            `Please enter 3 full relay teams (4 swimmers each) for ${event.name}.`
          );
          setHighlightMissing(true);
          return;
        }
    ...
    events.forEach((event) => {
      if (event.type === "individual") {
        const eligibleSwimmers = teamSwimmers
          .filter(
            (swimmer) =>
              swimmer.get(event.field) !== undefined &&
              swimmer.get(event.field) !== null
          )
          .map((swimmer) => ({
        ...
        if (event.name.includes("Medley")) {
          ...
          lineup[event.name] = strokes.map((stroke) => {
            const swimmer = teamSwimmers
              .filter(
                (swimmer) =>
                  swimmer.get(stroke) !== undefined &&
                  swimmer.get(stroke) !== null
              )
              .map((swimmer) => ({
                name:
          ...
          const eligibleSwimmers = teamSwimmers
            .filter(
              (swimmer) =>
                swimmer.get("free_100_time") !== undefined &&
                swimmer.get("free_100_time") !== null
            )
            .map((swimmer) => ({
              name: swimmer.get("first_name") + " " + swimmer.get("last_name"),
          ...
          const eligibleSwimmers = teamSwimmers
            .filter(
              (swimmer) =>
                swimmer.get("free_50_time") !== undefined &&
                swimmer.get("free_50_time") !== null
            )
            .map((swimmer) => ({
              name: swimmer.get("first_name") + " " + swimmer.get("last_name"),
                ...
                {event.type === "individual" ? (
                  [...Array(3)].map((_, i) => (
                    <input
                      ...
                      onChange={(e) =>
                        handleOpponentIndividualChange(
                          event.id,
                          i,
                          e.target.value
                        )
                      ...
                      style={
                        highlightMissing && !opponentTimes[event.id]?.[i]
                          ? { borderColor: "red" }
                          : {}
                      }
                    />
                      ...
                      >
                        {[...Array(4)].map((_, swimmerIdx) => (
                          <input
                            ...
                            type="text"
                            placeholder={`Relay ${relayIdx + 1} Swimmer ${
                              swimmerIdx + 1
                            }`}
                            value={
                              opponentTimes[event.id]?.[relayIdx]?.[
                                swimmerIdx
                              ] || ""
                            }
                            onChange={(e) =>
                              handleOpponentRelayChange(
                                event.id,
                                relayIdx,
                                swimmerIdx,
                                e.target.value
                            ...
                            style={
                              highlightMissing &&
                              !opponentTimes[event.id]?.[relayIdx]?.[swimmerIdx]
                                ? { borderColor: "red" }
                                : {}
                            }
                          />
            ...
            <div key={eventName} style={{ marginBottom: "10px" }}>
              ...
              <ul>
                {optimalLineup[eventName].map((swimmer, idx) => (
                  <li key={idx}>
                    {swimmer.name} - {swimmer.time.toFixed(2)}
                  </li>
                ))}
              </ul>
            </div>
